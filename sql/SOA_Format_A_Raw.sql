declare
	...
	SELECT DISTINCT
		...
		(
			--CASE [acc].[transactionDate]
			CASE DENSE_RANK() OVER (ORDER BY  acc.TransactionDate)--[acc].[transactionDate]
				--WHEN (@DateFrom)
				WHEN (1)
				--WHEN (MIN([acc].[transactionDate]))
				--WHEN ('10/1/2023 12:00:00 AM')
					THEN 
					(
						SELECT 
							(
								...
								+
								--=======================================================all current row/date Refund/Voided subheaders
								(
									--rv_ipp
									isnull
										(
