IF EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'pc_lst_acceptance'))
BEGIN
  PRINT 'Dropping...pc_lst_acceptance'
  DROP PROCEDURE pc_lst_acceptance
END
PRINT 'Creating...pc_lst_acceptance'
GO
				 
CREATE PROCEDURE pc_lst_acceptance
(
...
AS
	...
	SELECT 
		...
		'Encashment' AS [tagging],
		[acc].[officeCode], 
		FORMAT(CAST([acc].[transactionDate] AS DATETIME), 'dd-MMM-yyyy') AS [transactionDate], --'dd-MMM-yyyy HH:mm'
		[acc].[productType],
		[acc].[trackingNumber],
		[acc].[referenceNumber],
		...
		[acc].[unit],
		[acc].[principalAmount],
		FORMAT(CAST([acc].[encashmentDate] AS DATETIME), 'dd-MMM-yyyy') AS [encashmentDate],--'dd-MMM-yyyy HH:mm'
		[acc].[statusCode],
...
PRINT 'Creating...pc_lst_acceptance...complete'
GO
